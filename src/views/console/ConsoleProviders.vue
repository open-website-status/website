  ...
  export default class ConsoleProviders extends Vue {
    ...
    get providerItems () {
      return this.providers?.map(provider => {
        let placeholder = this.placeholderTokens.get(provider.id);
        if (placeholder === undefined) {
          placeholder = this.generateObviouslyRandomString(32);
          this.placeholderTokens.set(provider.id, placeholder);
        }
