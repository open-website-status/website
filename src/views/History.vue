<template>
  <v-main>
    <v-container
      ...
      v-if="urlError !== null"
    >
      <v-icon size="96" color="error">mdi-alert-circle</v-icon>
      <h4 class="text-h4 mt-8">The provided URL is invalid</h4>
      <h5 class="text-h5 mt-4" v-text="urlError" />
    </v-container>
    <v-container class="history-container" v-else-if="url !== null">
      <div class="history-url justify-center py-6 py-md-12">
        <HistoryURLElement label="Protocol">{{ url.protocol }}//</HistoryURLElement>
        <HistoryURLElement label="Hostname">{{ url.hostname }}</HistoryURLElement>
        <HistoryURLElement label="Port" v-if="url.port !== undefined">
          :{{ url.port }}
        </HistoryURLElement>
        <HistoryURLElement label="Pathname">{{ url.pathname }}</HistoryURLElement>
        <HistoryURLElement label="Search" v-if="url.search !== ''">
          {{ url.search }}
        </HistoryURLElement>
      </div>
      <v-expansion-panels hover :popout="$vuetify.breakpoint.mdAndUp">
        <v-expansion-panel v-for="query in queryItems" :key="query.id">
          <v-expansion-panel-header>
            <div class="mr-2 overflow-x-hidden">
              ...
              <div class="history-url mt-2">
                <HistoryURLElement label="Protocol" :different="url.protocol !== query.protocol">
                  {{ query.protocol }}//
                </HistoryURLElement>
                <HistoryURLElement label="Hostname">{{ query.hostname }}</HistoryURLElement>
                <HistoryURLElement
                  label="Port"
            ...
            <div>
                    ...
                    <v-card-title>
                      ...
                      <v-menu offset-y>
                        <template v-slot:activator="{ on }">
                          <v-btn icon v-on="on" class="info-button">
                            <v-icon>
                              mdi-information
                            </v-icon>
                          </v-btn>
                        ...
                        <v-list dense color="grey darken-3">
                          <v-list-item v-for="(item, i) in query.jobStatues" :key="i">
                            <v-sheet
                              :color="item.color"
                              height="24"
                              rounded
                  ...
                  <v-card outlined>
                    ...
                    <template v-if="query.timeoutCount > 0 || query.errorCounts.length > 0">
                      ...
                      <v-list subheader dense>
                        ...
                        <v-list-item v-for="error in query.errorCounts" :key="error.code">
                          <v-sheet
                            color="red"
                            height="24"
                            rounded
                    ...
                    <template v-if="query.httpCodeCounts.length > 0">
                      ...
                      <v-list subheader dense>
                        <v-list-item v-for="code in query.httpCodeCounts" :key="code.code">
                          <v-sheet
                            :color="code.color"
                            height="24"
                            rounded
              ...
              <div class="d-flex">
                ...
                <v-btn rounded outlined color="primary">
                  View details <v-icon right>mdi-arrow-right</v-icon>
                </v-btn>
              </div>
            </div>
          </v-expansion-panel-content>
